...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not query:
            await update.message.reply_text("🔍 اكتب ما تريد البحث عنه بعد الأمر:\n/search الرحمن")
            return
            
        await update.message.reply_chat_action(action="typing")
        results = await search_service.search_verses(query, max_results=7)
        await update.message.reply_text(results)
        
    except Exception as e:
        await update.message.reply_text("❌ فشل البحث، جرب كلمات مختلفة أو حاول لاحقاً")
...
async def tafsir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ فشل جلب التفسير، تحقق من التنسيق (مثال: 1:1)")
